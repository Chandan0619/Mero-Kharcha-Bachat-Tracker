...
def dashboard(request):
    ...
    total_automated_savings_gross = total_income * Decimal('0.20')
    
    # Deduct expenses explicitly paid from Savings
    savings_spent = Expense.objects.filter(user=request.user, source_type='Savings').aggregate(Sum('amount'))['amount__sum'] or 0
    total_automated_savings = total_automated_savings_gross - savings_spent
...
def add_income(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Create automatic savings (20%) - Now calculated dynamically in dashboard
